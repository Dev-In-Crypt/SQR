...
| RECEIPT_CONTRACT_ADDRESS | `SET` (`0x15e2D6a335aBBa7374ebeBa5EBD994346E2de35B`) | `LOCAL_ONLY_UNVERIFIED_FOR_LAUNCH` | `REQUIRES_DEPLOY_EVIDENCE` |
...
| OPENAI_GENERAL_MODEL | TODO (`UNVERIFIED`, `REQUIRES_USER_RUNTIME_SOURCE`). Repo default exists | TODO (`UNVERIFIED`, `REQUIRES_USER_RUNTIME_SOURCE`). Repo default exists | No (default exists) | General LLM model for summaries/formatting | `lib/config.ts` | `UNVERIFIED` | TODO | TODO |
| OPENAI_AUDIT_MODEL | TODO (`UNVERIFIED`, `REQUIRES_USER_RUNTIME_SOURCE`) | TODO (`UNVERIFIED`, `REQUIRES_USER_RUNTIME_SOURCE`) | No (falls back to OPENAI_GENERAL_MODEL) | Stronger model for AI audit stage | `lib/config.ts`, `lib/llm.ts` | `UNVERIFIED` | TODO | TODO |
| ENABLE_STRUCTURED_AUDIT_CONTEXT | TODO (`UNVERIFIED`, `REQUIRES_USER_RUNTIME_SOURCE`). Repo default: `true` | TODO (`UNVERIFIED`, `REQUIRES_USER_RUNTIME_SOURCE`). Repo default: `true` | No (default exists) | Backend-only structured context pre-step for AI audit | `lib/config.ts`, `lib/contract-structure.ts`, `lib/llm.ts` | `UNVERIFIED` | TODO | TODO |
